...
- Поддержка актуальных ограничений Squad на уровне конкретного LayerName: для каждой стороны слоя в `script.js` хранится список доступных фракций и их battlegroups / unit types из squadutils.org.
...
```txt
AdminChangeLayer Sumari_Seed_v1 WPMC+CombinedArms_Seed MEI+CombinedArms_Seed
```

Если Unit Type не выбран, команда формируется только с кодом фракции:

```txt
AdminChangeLayer Sumari_Seed_v1 WPMC MEI
```
...
## Обновление актуальных фракций и доктрин слоёв

Скрипт `scripts/fetch_layer_team_information.py` автоматически проходит по всем объектам массива `maps` в `script.js`, берёт `layer`, запрашивает `https://squadutils.org/api/v1/teamInformation?layerId=<LayerName>` и перезаписывает данные слоя в формате `teams`: доступные фракции и battlegroups / unit types отдельно для команды 1 и команды 2.
...
После обновления каждый слой в `script.js` выглядит так: базовые поля карты (`name`, `layer`, `mode`, `image`) плюс `teams`, где внутри каждой команды лежат фракции и их `allowedUnitTypes` из API.

Полезные опции:

```bash
# проверить все запросы, но не изменять script.js
python3 scripts/fetch_layer_team_information.py --dry-run

# посмотреть только один слой в читаемом виде
python3 scripts/fetch_layer_team_information.py Sumari_Seed_v1

# вывести сырой JSON одного слоя из API
python3 scripts/fetch_layer_team_information.py Sumari_Seed_v1 --json

# сохранить сырой JSON одного слоя в файл
python3 scripts/fetch_layer_team_information.py Sumari_Seed_v1 --output sumari_team_information.json
```

Если API не отдаёт информацию по конкретному LayerName, скрипт пишет предупреждение и оставляет для этого слоя пустой `teams`, чтобы остальные слои всё равно обновились. Скрипт использует только стандартную библиотеку Python, поэтому дополнительные зависимости устанавливать не нужно.
...
INS	Middle Eastern Insurgents / Повстанцы Ближнего Востока	Independent	AdminChangeLayer Sumari_Seed_v1 WPMC MEI	INS+Armored, INS+CombinedArms, INS+LightInfantry, INS+Mechanized, INS+Motorized, INS+Support
...
WPMC	Western Private Military Contractors / Западная ЧВК	Independent	AdminChangeLayer Sumari_Seed_v1 WPMC MEI	WPMC+AirAssault, WPMC+CombinedArms, WPMC+LightInfantry, WPMC+Motorized
