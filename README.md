...
- Поддержка актуальных ограничений Squad на уровне конкретного LayerName: для каждой стороны слоя в `data/layers.js` хранится список доступных фракций и их battlegroups / unit types из squadutils.org.
...
```txt
AdminChangeLayer Sumari_Seed_v1 WPMC+CombinedArms_Seed INS+CombinedArms_Seed
```
...
В `assets/maps` хранятся `.webp` миникарты. Пути в `data/layers.js` должны быть относительными web-путями и использовать прямой слеш `/`.
...
2. Добавьте новый объект в массив `maps` в `data/layers.js`:
...
Скрипт `scripts/fetch_layer_team_information.py` автоматически проходит по всем объектам массива `maps` в `data/layers.js`, берёт `layer`, запрашивает `https://squadutils.org/api/v1/teamInformation?layerId=<LayerName>` и перезаписывает данные слоя в формате `teams`: доступные фракции и battlegroups / unit types отдельно для команды 1 и команды 2.
...
После обновления каждый слой в `data/layers.js` выглядит так: базовые поля карты (`name`, `layer`, `mode`, `image`) плюс `teams`, где внутри каждой команды лежат фракции и их `allowedUnitTypes` из API.

Полезные опции:

```bash
# проверить все запросы, но не изменять `data/layers.js`
...
Если API не отдаёт информацию по конкретному LayerName, скрипт пишет предупреждение и оставляет для этого слоя пустой `teams`, чтобы остальные слои всё равно обновились. Код фракции `MEI` из старых/альтернативных данных API автоматически нормализуется в `INS`, потому что в приложении используется только актуальный код `INS`. Скрипт использует только стандартную библиотеку Python, поэтому дополнительные зависимости устанавливать не нужно.
